...
| A-0005 | Pydantic é adequado às fronteiras persistidas; o orchestrator final ainda deve ser escolhido por evidência entre plain async e LangGraph | W001-T006/T010; W002-T001/T005 | MEDIUM | MEDIUM | plain async lidera provisoriamente porque executou EXP-B; reexecutar challenger LangGraph em ambiente com dependência antes de lock | Architecture | PLAIN_ASYNC_PROVISIONAL_LANGGRAPH_PENDING_RECHECK |
...
| A-0008 | Documentos públicos-alvo poderão ser obtidos e parseados de forma confiável suficiente para demo | E-0001,W001-T010,W002-T002 | HIGH | MEDIUM | 3 famílias públicas testadas; exigir source/table-role provenance; ampliar raw-byte corpus e parser bakeoff antes de library lock | Data | PARTIALLY_SUPPORTED |
...
| A-0010 | LLM/semantic judge deve ser sensor secundário e estruturado, não árbitro único | W001-T004/T009/T010; W002-T006 | HIGH | LOW | hard gates determinísticos/source-first; semantic ablation EXP-G | Eval | CONTROLLED_BY_DESIGN |
...
|---|---|---|---|---|---|---|---|
| RISK-0001 | Alucinação ou drift factual em números, datas, entidades, direção/condicionais | HIGH | CRITICAL | claim sem source support ou anchor mismatch | T006 factual oracle + T007 deterministic anchors + later claim grounding | Eval | CONTROLLED_PARTIAL |
...
| RISK-0008 | Conteúdo gerado cruza fronteira de adaptação e vira recomendação financeira nova | MEDIUM | HIGH | buy/sell/price target não presente na fonte | T003 executable policy + adversarial suite; T008 integra ao domain core | Compliance | CONTROLLED_PARTIAL |
| RISK-0009 | Uma única função de qualidade é aplicada indevidamente a artigo/carrossel/vídeo | MEDIUM | MEDIUM | output passa métrica textual mas falha formato | T004 schemas por formato + T009 integration/tests | Eval/UX | CONTROLLED_PARTIAL |
| RISK-0010 | Extração de PDF perde tabelas/números/rodapés e contamina todo pipeline | MEDIUM | HIGH | anchor coverage/role provenance baixa | T002 source-trust contract; table-role provenance; fallback/review | Data | CONTROLLED_PARTIAL |
