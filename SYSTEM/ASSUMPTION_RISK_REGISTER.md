...
|---|---|---|---|---|---|---|---|
| A-0001 | Para o vídeo final, a regra operacional deve ser <=5:00 apesar de o Entregável 5 mencionar “5 a 7 minutos” | E-0001,E-0013,E-0015 | CRITICAL | MEDIUM | T017 produziu MP4 real medido em 7.200s <=300s; T018 valida suficiência evaluator-facing; confirmar com organizador se houver canal | Orchestrator | CONTROLLED_ACTUAL_RUNTIME_PASS_USABILITY_OPEN |
...
| A-0008 | Documentos públicos-alvo podem ser parseados de forma confiável suficiente para demo | W002-T002/T007/T010;W003-T009;W004-T003;E-0014,E-0015 | HIGH | MEDIUM | T014/T017 demonstraram recipient-facing real PDF/text input com raw hash/source-trust; raw-byte same-input cross-parser/OCR continuam necessários antes de parser lock | Data | SUPPORTED_DEMO_PATH_PARSER_IDENTITY_UNLOCKED |
...
| RISK-0006 | Vídeo não prova funcionamento real <=5 min ou é insuficiente para avaliador | MEDIUM | CRITICAL | artifact ausente, >5 min, ou gravação curta demais/ilegível para demonstrar fluxo | T017 real browser capture + 7.200s measured + DOM assertions; T018 fresh blind review decide evaluator-facing sufficiency | Demo | TECHNICAL_CAPTURE_CONTROLLED_FINAL_DEMO_USABILITY_PENDING_T018 |
...
| RISK-0010 | Extração PDF perde tabelas/números/rodapés | MEDIUM | HIGH | source-trust baixa | role-aware source gate catches wrong-role/unit corruption; T014/T017 fail-closed recipient path | Data | REDUCED_DETECTION_STRONG_PARSER_LOCK_OPEN |
...
| RISK-0016 | Fallback/demo parece conteúdo enlatado e não prova execução real | LOW-MEDIUM | HIGH | demo sem input/source/evidence path real | T017 recorded real browser interaction with real PDF and DOM assertions; T018 validates communication quality | Demo | STRONGLY_REDUCED_REAL_CAPTURE_REVIEW_PENDING |
...
| RISK-0021 | Mechanics proof ser confundido com provider/model-quality ou production-readiness proof | MEDIUM | CRITICAL | deterministic/CI mechanics usados em claim externo | cockpit states + T017 evidence boundaries + provider harness + final T008 gate | Orchestrator | STRONGLY_REDUCED_GUARDRAILS_ACTIVE |
...
| RISK-0024 | Pacote recipient-facing parece dashboard estático e não demonstra ingestão real | LOW-MEDIUM | HIGH | avaliador não encontra upload/input PDF/texto nem interação clara | T014 implementation + T017 real browser path; T018 re-review | UX/Data | MITIGATED_IMPLEMENTED_REVIEW_PENDING |
| RISK-0025 | Evidência fica fragmentada sem relatório experimental coerente | LOW-MEDIUM | HIGH | avaliador não reconstrói experimento/trade-offs/unknowns | T015 consolidated report/submission packet; T018 re-review | Writer/Auditor | MITIGATED_IMPLEMENTED_REVIEW_PENDING |
| RISK-0026 | Actions artifact de vídeo expira antes da avaliação/submissão | MEDIUM | HIGH | artifact `10652146281` expira `2026-12-20T16:24:02Z` sem cópia durável | se T018 aceitar o artifact, copiar para storage de submissão durável preservando MP4 SHA, artifact digest, run/task SHA e provenance | Orchestrator/Demo | OPEN_RETENTION_GATE |
