...
| RISK-0017 | Mudança de model/provider altera comportamento/custo | HIGH | MEDIUM | provider drift | provider-neutral harness + explicit N/A/pricing provenance; Groq identity observed but accepted real mechanics still blocked by permissions | Architecture | REDUCED_HARNESS_READY_GROQ_PERMISSION_BLOCKED |
...
| RISK-0022 | Provider credential/runtime permission impede evidência real de latency/usage/cost | MEDIUM | HIGH | Groq API/preflight retorna permission blocker | T010 secret-safe path + T004 A04/A05 confirmaram provider=Groq; A05 `/openai/v1/models` retorna HTTP 403 antes de geração. Corrigir org/project/key access no Groq Console e usar novo attempt | Orchestrator/Provider | EXTERNAL_GROQ_PERMISSION_BLOCKER_CHARACTERIZED |
