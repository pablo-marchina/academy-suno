...
| W005 DAG | T001..T009 parallel; T010→T011→T012 gated | PASS | sem ciclos/dependency ambiguity observada |
...
| Dependency manifest | production package/toolchain lock | INTENTIONALLY_PENDING | material choice deve sair de W005/DRG; não é blocker da pesquisa |
...
- as nove pesquisas iniciais são independentes e cobrem os principais P0/P1;
...
Ainda **não** é correto criar a arquitetura final de produção ou iniciar o bulk implementation da Phase 9 escolhendo, por preferência, frontend/auth/database/workflow/provider/observability/deployment.

O gate correto é:

```text
W005-T001..T009 evidence
        ↓
...
**Disposition:** explicitamente gated para T010/T012; a wave de implementação deve criar o manifest/toolchain reproduzível como um de seus primeiros increments depois da arquitetura aceita.
...
A partir do merge desta reconciliação, o início correto é abrir **W005-T001..T009 em paralelo**. Não é necessária uma nova fase de planejamento antes disso.

O Orchestrator deve reconstruir o lifecycle por `TASK_SIGNALS`/Issues/results, integrar micro-fan-ins válidos e manter T010 bloqueada até todos os inputs obrigatórios serem aceitos.
